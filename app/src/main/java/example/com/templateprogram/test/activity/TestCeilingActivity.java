...
import example.com.templateprogram.test.adapter.CeilingAdapter;
import example.com.templateprogram.test.bean.CeilingBean;
import example.com.templateprogram.test.bean.DataUtil;
import example.com.templateprogram.test.listener.PowerGroupListener;
import example.com.templateprogram.test.view.SectionDecoration;
...
public class TestCeilingActivity extends BaseActivity implements CeilingAdapter.OnItemClickListener {
    ...
    protected int getLayoutId() {
        return R.layout.activity_test_ceiling;
    }
    ...
    private void setPullAction() {
        ...
        if (beanList != null && beanList.size() > 0) {
            for (int i = 0; i < beanList.size(); i++) {
                ...
                if ("类别一".equals(gName)) {
                    data.setName("最新类别1");
                    data.setIcon(R.mipmap.ic_text);
                } else if ("类别二".equals(gName)) {
                    data.setName("最新类别2");
                    data.setIcon(R.mipmap.ic_picture_pressed);
                } else if ("类别三".equals(gName)) {
                    data.setName("最新类别3");
                    data.setIcon(R.mipmap.ic_video_pressed);
                }
                dataList.add(data);
            }
        }
    ...
    private void initDecoration() {
        SectionDecoration decoration = SectionDecoration.Builder
                ...
                //设置高度
                .setGroupHeight(ScreenUtils.dip2px(this, 40))
                .build();
